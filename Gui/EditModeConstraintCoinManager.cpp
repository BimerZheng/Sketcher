    ...
    for (std::vector<Sketcher::Constraint*>::const_iterator it = constrlist.begin();
        ...
        try {  // because calculateNormalAtPoint, used in there, can throw
            ...
            switch (Constr->Type) {
                ...
                case Slope: {
                    // Slope constraint applies to a single line. Show the
                    // numerical slope value via a SoDatumLabel placed at the
                    // line's midpoint. We reuse the DISTANCE datum type but
                    // pass the same point twice as endpoints so only the
                    // numeric value is rendered (no extension line / arrow).
                    assert(Constr->First >= -extGeoCount && Constr->First < intGeoCount);

                    Base::Vector3d midpos, dir, norm;

                    if (Constr->Second == GeoEnum::GeoUndef) {
                        // slope of a single line
                        const Part::Geometry* geom = geolistfacade.getGeometryFromGeoId(Constr->First);
                        if (!geom || !geom->is<Part::GeomLineSegment>()) {
                            break;
                        }
                        const Part::GeomLineSegment* lineSeg = static_cast<const Part::GeomLineSegment*>(geom);
                        Base::Vector3d dirvec = lineSeg->getEndPoint() - lineSeg->getStartPoint();
                        midpos = lineSeg->getStartPoint() + dirvec / 2;
                    ...
                    SbVec3f* verts = asciiText->pnts.startEditing();
                    SbVec3f pos(midpos.x + relpos.x, midpos.y + relpos.y, zConstrH);
                    verts[0] = pos;
                    verts[1] = pos;
                    asciiText->pnts.finishEditing();
                } break;
...
)
{
    ...
    for (std::vector<Sketcher::Constraint*>::const_iterator it = constrlist.begin();
        ...
        switch ((*it)->Type) {
            ...
            case Slope: {
                // Slope is a dimensional constraint for a single line. We use
                // a SoDatumLabel so that the numerical slope value is shown
                // (mirroring the Angle/Distance labels). The label node is at
                // DatumLabelIndex (0); no SoZoomTranslation/SoImage is needed
