    ...
    switch (constraint->Type) {
        ...
        case Slope:
            ...
            // slope of a single line
                ...
                rtn = addSlopeConstraint(constraint->First, c.value, c.driving);
            }
            break;
        case Radius: {
            c.value = new double(constraint->getValue());
...
// slope constraint on a single line
int Sketch::addSlopeConstraint(int geoId, double* value, bool driving)
{
    geoId = checkGeoId(geoId);
    ...
    if (Geoms[geoId].type == Line) {
        ...
        int tag = ++ConstraintsCounter;
        GCSsys.addConstraintSlope(l, value, tag, driving);
        return ConstraintsCounter;
    }
    return -1;
}

// slope constraint between two points
int Sketch::addSlopeConstraint(int geoId1, PointPos pos1, int geoId2, PointPos pos2, double* value, bool driving)
{
    geoId1 = checkGeoId(geoId1);
    geoId2 = checkGeoId(geoId2);
    ...
    // slope of the vector p1->p2; the direction is taken from p1 to p2
    int tag = ++ConstraintsCounter;
    GCSsys.addConstraintSlope(*l1p1, *l1p2, value, tag, driving);
    return ConstraintsCounter;
}
