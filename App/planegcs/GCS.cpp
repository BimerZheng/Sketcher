...
int System::addConstraintSlope(Point& p1, Point& p2, double* slope, int tagId, bool driving)
{
    Constraint* constr = new ConstraintSlope(p1, p2, slope);
    constr->setTag(tagId);
    constr->setDriving(driving);
    return addConstraint(constr);
}

int System::addConstraintSlope(Line& l, double* slope, int tagId, bool driving)
{
    Constraint* constr = new ConstraintSlope(l, slope);
    constr->setTag(tagId);
    constr->setDriving(driving);
    return addConstraint(constr);
}
    ...
    for (std::vector<Constraint*>::iterator it = clist_.begin(); it != clist_.end(); ++it, ++ic) {
        ...
        switch ((*it)->getTypeId()) {
            ...
            case PointOnEllipse: {  // 7
                ...
                // Clothoid constraints
                CASE_NOT_IMP(PointOnClothoid)
                CASE_NOT_IMP(ClothoidG0Continuity)
                CASE_NOT_IMP(ClothoidG1Continuity)
                CASE_NOT_IMP(ClothoidG2Continuity)
                CASE_NOT_IMP(ClothoidCurvature)
                CASE_NOT_IMP(Snell)
                CASE_NOT_IMP(None)
        }
    }
