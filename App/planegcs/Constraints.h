...
// Slope
// 约束线段 p1->p2 的坡率（坡度）等于给定值 slope。
// 方向无关定义：error = dy - slope*|dx| = 0，其中 dy=p2.y-p1.y, dx=p2.x-p1.x。
// 坡率 slope 只由竖直落差 dy 的符号决定：dy>0（上坡）时 slope>0，dy<0（下坡）时 slope<0，
// 与点序的左右方向（dx 符号）无关。因此道路左右两侧的对称边坡可统一传正值（上坡）。
// 注意：本约束描述的是"坡率"而非"带符号斜率"，竖直段（dx≈0）坡率趋于无穷，应避免。
class ConstraintSlope: public Constraint
{
private:
    inline double* p1x()
    {
...
public:
    ConstraintSlope(Point& p1, Point& p2, double* s);
    ConstraintSlope(Line& l, double* s);
#ifdef _GCS_EXTRACT_SOLVER_SUBSYSTEM_
    inline ConstraintSlope()
    {}
#endif
    virtual ConstraintType getTypeId();
    virtual void rescale(double coef = 1.);
