...
// Slope
// 坡度约束：约束线段 p1->p2 的坡率 slope = (p2.y-p1.y)/(p2.x-p1.x) 等于给定值。
// 采用线性表述 error = dy - slope*dx：无量纲化前的误差为长度量纲，
// 但与线段绘制方向无关（左坡/右坡均可），正坡率为上坡、负坡率为下坡，
// 适合道路边坡（填方/挖方坡率）等场景。
ConstraintSlope::ConstraintSlope(Point& p1, Point& p2, double* s)
{
    pvec.push_back(p1.x);
    pvec.push_back(p1.y);
    pvec.push_back(p2.x);
...
ConstraintSlope::ConstraintSlope(Line& l, double* s)
{
    pvec.push_back(l.p1.x);
    pvec.push_back(l.p1.y);
    pvec.push_back(l.p2.x);
...
ConstraintType ConstraintSlope::getTypeId()
{
    return Slope;
}

void ConstraintSlope::rescale(double coef)
{
    scale = coef * 1.;
}

double ConstraintSlope::error()
{
    // 方向无关的坡率约束：error = dy - s*|dx| = 0
    // 坡率 s 只由竖直落差 dy 的符号决定（dy>0 上坡 s>0，dy<0 下坡 s<0），
    // 与点序 p1->p2 的左右方向（dx 符号）无关。故左右对称边坡可统一传 +s。
    double dx = (*p2x() - *p1x());
    double dy = (*p2y() - *p1y());
    double s = *slope();
    return scale * (dy - s * std::abs(dx));
}

double ConstraintSlope::grad(double* param)
{
    double deriv = 0.;
    double dx = (*p2x() - *p1x());
    double dy = (*p2y() - *p1y());
    double s = *slope();
    // error = dy - s*|dx|，其中 d(|dx|)/dx1 = -sign(dx)，d(|dx|)/dx2 = +sign(dx)
    // 当 |dx| 很小（dx≈0）时 sign 取值不影响梯度，取 sign=+1 即可
    double sgn = (dx >= 0.0) ? 1.0 : -1.0;
    double absdx = std::abs(dx);
    if (param == p1x()) {
        deriv += s * sgn;  // ∂/∂x1 = +s*sign(dx)
    }
    if (param == p2x()) {
        deriv += -s * sgn;  // ∂/∂x2 = -s*sign(dx)
    }
    if (param == p1y()) {
        deriv += -1.;  // ∂/∂y1 = -1
    }
...
double ConstraintSlope::maxStep(MAP_pD_D& dir, double lim)
{
    // 限制坡率参数变化，使其引起的角度变化 <= pi/18 = 10°
    MAP_pD_D::iterator it = dir.find(slope());
    if (it != dir.end()) {
        double s = *slope();
        double angleStep = std::abs(it->second) / (1. + s * s);
        if (angleStep > M_PI / 18.) {
            lim = std::min(lim, (M_PI / 18.) / angleStep);
        }
    }
    return lim;
}
