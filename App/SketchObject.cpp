...
int SketchObject::getSingleScaleDefiningConstraint() const
{
    ...
    int found = -1;
    for (size_t i = 0; i < vals.size(); ++i) {
        // Only true length-defining constraints (not dimensionless ratios or angles)
        // can serve as a scale reference.  Angle, Slope and SnellsLaw are
        // dimensionless and would cause geometry to be scaled incorrectly if
        // (mis)treated as scale-defining.
        if (vals[i]->isDimensional() && vals[i]->Type != Angle
            && vals[i]->Type != Slope && vals[i]->Type != SnellsLaw) {
