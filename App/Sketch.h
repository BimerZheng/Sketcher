...
public:
    ...
    /**
     *   add a slope constraint on a line: constrains the slope (dy/dx) of the
     *   line's direction vector to the given value.
     *
     *   double * value is a pointer to double allocated in the heap, containing the
     *   constraint value and already inserted into either the FixParameters or
     *   Parameters array, as the case may be.
     */
    int addSlopeConstraint(int geoId, double* value, bool driving = true);
    /**
     *   add a slope constraint between two points: constrains the slope
     *   (dy/dx) of the vector from the first to the second point.
     *
     *   double * value is a pointer to double allocated in the heap, containing the
     *   constraint value and already inserted into either the FixParameters or
     *   Parameters array, as the case may be.
     */
    int addSlopeConstraint(
        int geoId1,
        PointPos pos1,
        int geoId2,
        PointPos pos2,
